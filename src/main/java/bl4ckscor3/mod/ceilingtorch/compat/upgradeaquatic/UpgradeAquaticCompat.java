...
public class UpgradeAquaticCompat implements ICeilingTorchCompat {
	...
	public UpgradeAquaticCompat() {
		for (JellyTorchType type : JellyTorchType.values()) {
			...
			RegistryObject<JellyCeilingTorchBlock> ceilingTorch = CeilingTorch.BLOCKS.register(String.format("upgrade_aquatic_%s_jelly_torch", typeName), () -> new JellyCeilingTorchBlock(BlockBehaviour.Properties.copy(Blocks.TORCH).sound(SoundType.METAL), type, type.torch));

			CEILING_TORCHES.add(ceilingTorch);
		}

		DistExecutor.safeRunWhenOn(Dist.CLIENT, () -> UpgradeAquaticCompatClient::addListeners);
	}
