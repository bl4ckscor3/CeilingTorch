...
public class ColoredTorchesCompat implements ICeilingTorchCompat {
	...
	private static DeferredBlock<ColoredCeilingTorchBlock> registerCeilingTorch(DyeColor color) {
		...
		DeferredHolder<Block, Block> coloredTorch = DeferredHolder.create(ResourceKey.create(Registries.BLOCK, Identifier.fromNamespaceAndPath("colored_torches", name)));

		return CeilingTorch.BLOCKS.registerBlock("colored_torches_" + name, p -> new ColoredCeilingTorchBlock(p, color, coloredTorch), ColoredTorchesCompat::prop);
	}

	private static BlockBehaviour.Properties prop() {
		return BlockBehaviour.Properties.of().noCollision().instabreak().lightLevel(state -> 14).sound(SoundType.WOOD).pushReaction(PushReaction.DESTROY);
	}
