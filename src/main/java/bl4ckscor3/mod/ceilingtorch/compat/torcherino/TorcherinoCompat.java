...
public class TorcherinoCompat implements ICeilingTorchCompat
{
	...
	public void registerBlocks(Register<Block> event)
	{
		TorcherinoAPI.INSTANCE.getTiers().keySet().forEach(tierId -> {
			...
			{
				ResourceLocation flameId = loc("torcherino", tierId, "flame");
				CeilingTorcherinoBlock block = (CeilingTorcherinoBlock)new CeilingTorcherinoBlock(tierId, flameId).setRegistryName(loc(CeilingTorch.MODID, "torcherino_", tierId, "torcherino"));
