...
public class TorchBandolierCompat implements ICeilingTorchCompat {
	...
	public void onRightClickBlock(PlayerInteractEvent.RightClickBlock event) {
		if (event.getFace() == Direction.DOWN && event.getItemStack().getItem() instanceof TorchBandolierItem) {
			...
			ResourceLocation rl = getRegistryName(torch.asItem());
			...
			if (compatList.containsKey(modid)) {
				...
				if (placeEntries.containsKey(rl) && PlaceHandler.placeTorch(compat, event, ItemStack.EMPTY, event.getPos().relative(event.getFace()), event.getLevel(), placeEntries.get(rl).defaultBlockState())) {
					if (consumeTorch)
