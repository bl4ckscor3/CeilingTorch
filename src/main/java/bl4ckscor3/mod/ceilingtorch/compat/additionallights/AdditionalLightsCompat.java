...
public class AdditionalLightsCompat implements ICeilingTorchCompat {
	...
	static {
		...
		for (ModBlockList block : supportedBlocks) {
			...
			registeredBlock = CeilingTorch.BLOCKS.registerBlock(registryName, p -> new ALCeilingTorchBlock(p, originalBlock), BlockBehaviour.Properties.of().noCollission().instabreak());
