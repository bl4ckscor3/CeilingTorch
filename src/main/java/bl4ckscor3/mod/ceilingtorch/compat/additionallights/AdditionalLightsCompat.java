...
public class AdditionalLightsCompat implements ICeilingTorchCompat {
	...
	static {
		...
		for (ModBlockList block : supportedBlocks) {
			...
			registeredBlock = CeilingTorch.BLOCKS.register(registryName, () -> new ALCeilingTorchBlock(BlockBehaviour.Properties.of().noCollision().instabreak().setId(id), originalBlock));
