...
public class AdditionalLightsCompat implements ICeilingTorchCompat {
	...
	static {
		...
		for (ModBlockList block : supportedBlocks) {
			DeferredHolder<Block, Block> originalBlock = AdditionalLights.modBlocks.get(block);
			...
			id = ResourceKey.create(BuiltInRegistries.BLOCK.key(), ResourceLocation.fromNamespaceAndPath(CeilingTorch.MODID, registryName));
			registeredBlock = CeilingTorch.BLOCKS.register(registryName, () -> new ALCeilingTorchBlock(BlockBehaviour.Properties.of().noCollission().instabreak().setId(id), originalBlock));
