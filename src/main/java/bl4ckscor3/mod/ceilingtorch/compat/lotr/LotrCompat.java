...
public class LotrCompat implements ICeilingTorchCompat
{
	...
	public void registerBlocks(RegistryEvent.Register<Block> event)
	{
		...
		event.getRegistry().register(ceilingBlubberTorch = new LotrCeilingTorchBlock(
				Block.Properties.create(Material.MISCELLANEOUS).doesNotBlockMovement().hardnessAndResistance(0.0F).setLightLevel(state -> 14).sound(SoundType.WOOD),
		...
		event.getRegistry().register(ceilingOrcTorch = new CeilingOrcTorchBlock(
				Block.Properties.create(Material.MISCELLANEOUS).doesNotBlockMovement().hardnessAndResistance(0.0F).setLightLevel(state -> state.get(CeilingOrcTorchBlock.HALF) == DoubleBlockHalf.LOWER ? 12 : 0).sound(SoundType.WOOD))
