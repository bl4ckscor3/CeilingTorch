...
public class ShardCeilingTorchBlock extends CeilingTorchBlock {
	...
	public void tick(BlockState state, ServerLevel level, BlockPos pos, RandomSource random) {
		level.getEntitiesOfClass(LivingEntity.class, new AABB(pos).inflate(4.0D), entity -> entity.is(UGTags.Entities.ROTSPAWN)).forEach(entity -> {
			Vec3 center = pos.getCenter();

			if (entity.hurtServer(level, UGDamageSources.getShardTorchDamage(level, center), 4.0F))
				ShardTorchBlock.drawParticlesTo(level, center, entity);
		});
		level.scheduleTick(pos, this, 20);
	}
